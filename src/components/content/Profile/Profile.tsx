...
export default function Profile() {
  ...
  const { data: avatar, isLoading: isAvatarLoading } = useGetImageQuery(
    data?.avatar || '',
    { skip: !data?.avatar },
  )
  const [addFriend, { isLoading: isLoadingAddFriend }] = useAddFriendMutation()
  const [removeFriend, { isLoading: isLoadingRemoveFriend }] =
    useRemoveFriendMutation()
  ...
  return (
    <div>
      <h2 className='page-title'>
        {isMyProfile ? 'My profile' : `${data?.name || 'User'} profile`}
      </h2>
